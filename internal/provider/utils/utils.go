...
// DescribeV20260501 parses descriptions for a component from its schema in
// Smallstep's 2026-05-01 OpenAPI spec. This ensures the terraform attribute
// documentation is kept in sync with the API spec.
func DescribeV20260501(component string) (string, map[string]string, error) {
	spec, err := v20260501.GetSwagger()
	if err != nil {
		return "", nil, err
	}
	...
	componentSchema, ok := spec.Components.Schemas[component]
	if !ok || componentSchema.Value == nil {
		return "", nil, fmt.Errorf("no schema found for %q in OpenAPI spec (2026-05-01)", component)
	}
