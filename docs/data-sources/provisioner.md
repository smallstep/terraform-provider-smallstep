...
- `x5c` (Attributes) A [provisioner](https://smallstep.com/docs/step-ca/provisioners/#x5c---x509-certificate) that authenticates a certificate request with an existing X.509 certificate. This object is populated when type is `X5C`. (see [below for nested schema](#nestedatt--x5c))
